...
const { decompress } = require('./huffman');  // Local in functions/
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!fileName.toLowerCase().endsWith('.zip')) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Only ZIP files are allowed' }) };
    ...
    if (!combinedData) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Invalid compressed file' }) };
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Decompression failed', message: error.message }) };
  }
};
